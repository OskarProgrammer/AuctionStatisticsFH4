export const HistoryTab = ( props ) => {
    ...
    return(
        <div className={`container ${auction.status == "success" ? "bg-success": "bg-danger"} text-white rounded border-dark border-2 p-3 my-3`}>
            ...
            {auction.status == "success" ? <p className="fs-4">Sold for {auction.finalPrice}</p> : ""}
            {auction.finalPrice == auction.buyoutCost ? <p className="fs-4">Method of purchase: buyout</p> : <p className="fs-4">Method of purchase: bidding</p>}
        </div>
    )
}